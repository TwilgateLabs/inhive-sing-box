...
func (to *TLSObfs) read(b []byte, discardN int) (int, error) {
	...
	if err != nil {
		// Было `return 0, nil` — ошибка чтения длины записи выбрасывалась и
		// подменялась ФИКТИВНЫМ успехом. (0, nil) из Read — нарушение
		// контракта io.Reader: для io.Copy это не сигнал завершения, он
		// просто зовёт Read снова. При этом заголовок записи (3 байта выше)
		// уже съеден, то есть поток десинхронизирован — следующий Read
		// откусывает данные с середины и отдаёт мусор.
		// Со стороны наблюдателя: ss+obfs=tls «висит» или сыпет мусором, в
		// логах — ни одной строки, потому что ошибки формально не было.
		// Строкой выше (37) идентичный случай уже возвращается честно —
		// расхождение внутри одной функции.
		return 0, err
	}
